...
export default function EventDetailClient({ category, details }: Props) {
  ...
  const [dbEventDeadline, setDbEventDeadline] = useState<string | null>(null);
      ...
      .then((data) => {
        if (data.success) {
          const match = data.data.items.find(
            (e: { id: string; name: string; deadline: string | null }) => e.name === category.eventName,
          );
          if (match) {
            setDbEventId(match.id);
            setDbEventDeadline(match.deadline);
          }
        }
      })
      .catch(() => {});
  ...
  if (dbEventDeadline) {
    const deadline = new Date(dbEventDeadline);
    const now = new Date();
            ...
            <div className="bg-white p-6 md:p-8 rounded-2xl border border-gat-blue/10 shadow-sm">
              ...
              <div className="space-y-4">
                ...
                <div className="pt-2 rounded-2xl border border-gat-blue/10 p-4">
                  {isDeadlineToday && !isDeadlineOver && (
                    <div className="w-full mb-4 py-2 px-3 text-xs font-bold text-center bg-yellow-100/80 text-yellow-800 uppercase tracking-widest border border-yellow-200 rounded-lg">
                      Closing soon: {formattedDeadline}
                    </div>
                  )}

                  {isDeadlineOver ? (
                    <div className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl font-bold text-sm bg-red-50 text-red-600 border border-red-200">
                      Deadline is over
                    </div>
                  ) : isLoggedIn ? (
                    inCart ? (
