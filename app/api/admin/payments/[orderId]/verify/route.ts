...
// POST /api/admin/payments/:orderId/verify — Verify payment and create registrations
export async function POST(req: NextRequest, context: RouteContext) {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      for (const item of order.orderItems) {
        if (item.Team && item.Team.members.length > 0) {
          // TEAM event: register every team member
    ...
    console.error("[POST /api/admin/payments/:orderId/verify]", error);
